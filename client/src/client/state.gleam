//// Application-state types, compile-time constants, and pure
//// model-mutation helpers. This module sits at the foundation of the
//// client's dependency graph: it depends only on `client/types`,
//// `client/pagination`, `client/navigation`, and `shared/segmenter`,
//// and is imported by every other client module that touches the
//// model.
////
...
////   `compute_current_chapter_title`).
//// * Writing `text` requires writing `total_sentence_count` /
////   `total_word_count` (refresh via `total_counts`).
////
//// The pure helpers here (`go_to_page`, `change_page`) refresh the
//// cached fields as they mutate, so callers that go through these
//// helpers do not need to worry about drift. Direct `Model(..model,
//// pages: ...)` updates in the reducer must mirror the invariant by
//// hand.
