...
import client/msg.{
  type Msg, ApplyFullSweep, ResetBookSettings, SetDeletionGranularity,
  SetDeletionIntensity, SetFontSize, SetGhostOpacity, SetLineSpacing, SetMode,
  SetPageDelay, SetParagraphDelay, SetWpm, ToggleDarkMode, ToggleDyslexiaFont,
  ToggleGhostMode, TogglePageDelete, ToggleSettings,
}
import client/state.{
  type DeletionGranularity, type DeletionIntensity, type Model, DeletePhrase,
  DeleteSentence, DeleteWord, High, Low, Manual, Medium, Reader, RealTime,
  max_font_size, max_ghost_opacity, max_line_spacing, max_page_delay_ms,
...
fn view_settings_sheet(model: Model) -> Element(Msg) {
  html.div([attribute.class("settings-panel"), stop_click_propagation()], [
    ...
    view_random_delete_section(model),
    view_book_override_section(model),
  ])
}

/// Random destructive deletion controls. Like the per-book override
/// section, these render only in the reader view with an active book —
/// the page-per-page seed and the full-sweep scope both need a loaded
/// book, and the settings are persisted per book. The section bundles
/// the two deliberately-different affordances (a persistent toggle and
/// a fire-once button) with the shared granularity / intensity pickers.
fn view_random_delete_section(model: Model) -> Element(Msg) {
